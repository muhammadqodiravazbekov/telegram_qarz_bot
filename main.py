...
async def webhook(req: Request):
    ...
    txt=msg.get("text","").split("@")[0]
    tid=msg.get("message_thread_id")
    if cid!=ALLOWED_GROUP: return {"ok":True}
    if txt in("/start","/royhati"):
        await tgsend(cid,{"chat_id":cid,"text":"👇 Tugmani bosing:","reply_markup":{"inline_keyboard":[[{"text":"📋 Qarzdorlar","web_app":{"url":WEBAPP_URL}}]]}},tid)
    elif txt=="/jami":
        c=db()
        ...
        await tgsend(cid,{"chat_id":cid,"text":"📊 *Qarzdorlar*\n\n"+"\n".join(lines)+f"\n\n💰 *Jami: {int(grand):,} so'm*".replace(",","  "),"parse_mode":"Markdown"},tid)
    return {"ok":True}

class PC(BaseModel):
    name:str
class TC(BaseModel):
    person_id:int; amount:float; note:Optional[str]=None; added_by:Optional[str]="Sotuvchi"
...
def cp(b:PC):
    ...
    c=db()
    try:
        cur=c.execute("INSERT INTO persons(name)VALUES(?)",(name,)); c.commit(); pid=cur.lastrowid
    except sqlite3.IntegrityError:
        c.close(); raise HTTPException(400,"Bu ism mavjud")
